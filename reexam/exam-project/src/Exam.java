...
/*
This is the re-exam for DM563 - Concurrent Programming, Spring 2020.

IMPORTANT: the methods have the same names as in the exam, but the descriptions are a bit different. Read them carefully!
...
and concurrency must be dealt with correctly (results cannot be wrong due to bad programming of concurrency).
...
https://github.com/fmontesi/cp2020/tree/master/reexam

The webpage also contains more rules about the reexam.
*/
public class Exam {
	...
	/** Returns the shortest word that does not start with "l" or "L" present in the
	 ...
	 * subdirectories, etc.).
	 * 
	 * Words starting with the letters "l" or "L" must not be considered: when you meet
	 * them, you must skip them. For example, if your possible words are "hello" and "low", then "hello" should
	 * be returned. If no words are available or only words starting with "l" or "L" are available,
	 * you should return null.
	 * 
	 * If no valid words can be found, the method must return null.
	 *
	...
	/** Returns the longest word that does not end with "l" or "L" present in the
	 * text files contained in a directory. (Notice that here we care about words not ending
	 * with l or L, whereas before in shortestWord we cared about them not starting with l or L.)
	 * 
	 * This method recursively visits a directory to find all the text files
	 * contained in it and its subdirectories (and the subdirectories of these
	 * subdirectories, etc.).
	 * 
	 * Words ending with the letters "l" or "L" must not be considered: when you meet
	 * them, you must skip them. For example, if your possible words are "pi" and "pal",
	 * then "pi" should be returned. If no words are available or only words ending
	 * with "l" or "L" are available, you should return null.
	 * 
	 * If no valid words can be found, the method must return null.
	 *
	 * You must consider only files ending with a ".txt" suffix. You are
	...
	/** Returns an Optional<LocatedWord> (see below) about a word
	 * starting with the given prefix found in the files of the given directory.
	 * 
	 * The returned word must not be longer than 5 characters. If all found words
	 * starting with the given prefix are longer than 5, then this method
	 * should return an empty optional (Optional.empty()).
	 * 
